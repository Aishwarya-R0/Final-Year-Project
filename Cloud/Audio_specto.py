...
import numpy as np
import cv2
output_folder = 'raw_data_img'  # set your output folder and make sure it exists
...
def plot_audio_spectrogram(audio_path, binsize=2**10, plot_path=None, argv = '', colormap="jet"):
    ...
    listed=['406_merged.wav', '417_merged.wav','420_merged.wav', '436_merged.wav']
    for folder in folder_list:
        ...
        for file in img_files:
            if file in (listed):
                print(file)
                out = fixed_len_segments(os.path.join(path, file), return_segments = False, save_path = (os.path.join(path)), save = True, segment_size = 30)
...
'''
os.chdir('dataset')
genres = 'Normal Depression'.split()
for idx,genre in tqdm(enumerate(genres),total=len(genres)):
    for fname in os.listdir(genre):
        
        file= genre+'/'+fname
        
        ims = plot_audio_spectrogram(file, 2**10, './'+output_folder + '/'+ ntpath.basename(fname.replace('.wav','')) + '.png',  's')
        os.chdir('dataset')
os.chdir('..')
...
for folder in folder_list:
        ...
        for file in img_files:
            ...
            print(src)
            ims = plot_audio_spectrogram(src , 2**10, path1+'/'+ntpath.basename(file.replace('.wav','')) + '.png',  's')
            #cv2.imwrite(os.path.join(path1, file),img)

            

folder_list = os.listdir('raw_data_img')


for folder in folder_list:
        
        # create a path to the folder
        path = './raw_data_img/' + str(folder)
        path1 = './raw_data_img/' + str(folder)
        img_files = os.listdir(path)
        print(path)
        for file in img_files:
