...
train_files, test_files, train_targets, test_targets = train_test_split(train_files, train_targets, test_size=0.30, shuffle= True)
...
burn_classes = [item[15:-1] for item in sorted(glob("./raw_data_img/*/"))]
...
model.summary()
'''
model.add(LSTM(units = 300,return_sequences = True,input_shape=(64, 3),
                         activation="tanh",
...
hist=model.fit(train_tensors, train_targets , validation_data=(test_tensors, test_targets), epochs=epoch, batch_size=64)

show_history_graph(hist)
train_loss, train_acc = model.evaluate(train_tensors, train_targets)
print("Training accuracy: "+str(train_acc))
test_loss, test_acc = model.evaluate(test_tensors, test_targets)
print("Test accuracy: "+str(test_acc))
x_pred=model.predict(train_tensors)
y_pred=model.predict(test_tensors)
from sklearn.metrics import confusion_matrix,accuracy_score
cm = confusion_matrix(np.argmax(train_targets, axis=1),np.argmax(x_pred, axis=1))
